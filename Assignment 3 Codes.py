#Question1-Counting the number of occurrences of each word or character

input_value = input("Enter the string: ").lower().replace("."," ").replace(","," ").split()      #Replace function is used to eliminate full stop and comma from the sentence,in case
...
for i in occurences:
    print('"%s" : %d' % (i,occurences[i]))

    
#Question2-Printing next date of input date
...
while True:
    ...
    if (date < 1) or (date > 31) or (month < 1) or (month > 12) or (year < 1800) or (year > 2025):                  #Condition for given constraints in the question
        print("Use the given conditions for entering the current date\nC1 : 1<=month<=12\nC2 : 1<=day<=31\nC3 : 1800<=year<=2025")
        continue
...
list_out = []
for i in list_in:
    list_out.append((i, i**2))              #(i, i**2) is the tuple which is added in the list
print("Output:", list_out)

#Question4-Grading
table = [["A+","Outstanding",10],
         ["A","Excellent",9],
         ["B+","Very Good",8],
         ["B","Good",7],
         ["C+","Average",6],
         ["C","Below Average",5],
         ["D","Poor",4]]
while True:                                                                      #loop for the user input between 4 and 10
    grade_point = eval(input("Enter the grade point of the student: "))
    if 4 <= grade_point <= 10:
        break
    else:
        print("The grade point must be between 4 and 10")
for i in table:                                                                  #i is for iterating through the lists in the list and j is for iterating through the elements of each list
    for j in i:
        if j == int(grade_point):
            print("Your Grade is '%s' and %s Performance" % (i[0],i[1]))
            break

#Question5- Printing pattern
string = "ABCDEFGHIJK"
j = 0
...
for sorted_value in sorted(dict1.values()):                             #Sorting the dictionary using student names
    for key,value in dict1.items():
        if value == sorted_value:
            dict2[key] = value
print("\nb. Student Details (sorted with respect to names):")           #(b)
for i in dict2:
...
for sorted_key in sorted(dict1):                                        #Sorting the dictionary using SIDs
    for key,value in dict1.items():
        if key == sorted_key:
            dict3[key] = value
print("\nc. Student Details (sorted with respect to SIDs):")            #(c)
for i in dict3:
...
#Question7-Printing Fibonacci sequence and its average
'''a is first number, b is second number and c is the extra variable used for adding and reassigning the values'''
a = 0
b = 1
sum = 0
while True:                                                                     #While loop for asking the user to input the value again if user enters a -ve value
...
set2 = {2,4,6,8}
set3 = {1,5,9,13,17}
print("a. Set of all elements that are in Set1 and Set2 but not in both:",end=" ")
set4 = set1^set2
print(set4)
